...
class Controller:
    ...
    def update_fingers_status():
        ...
        if Controller.all_fingers_up:
            Controller.gesture_description = "All fingers up"
        elif Controller.all_fingers_down:
            Controller.gesture_description = "All fingers down"
        elif Controller.index_finger_up and Controller.middle_finger_down:
            Controller.gesture_description = "Cursor moving"
        elif Controller.index_finger_up and Controller.middle_finger_up:
            Controller.gesture_description = "Scrolling"
        else:
            Controller.gesture_description = "Unknown gesture"
    ...
    def cursor_moving(x, y):
        # Calculate the average position of the palm landmarks (0, 1, 5, 9, 13, 17)
        palm_points = [0, 1, 5, 9, 13, 17]
        avg_x = sum(Controller.hand_Landmarks.landmark[point].x for point in palm_points) / len(palm_points)
        avg_y = sum(Controller.hand_Landmarks.landmark[point].y for point in palm_points) / len(palm_points)

        # Convert the average position to screen coordinates
        h, w, _ = Controller.img_shape
        x, y = Controller.get_position(avg_x, avg_y)
